...
result_t waitForData(size_t data_count, uint64_t timeout_ms, size_t *returned_size)
{
    ...
    while (true)
    {
        ...
        if ((millis() - start) >= timeout_ms) {
            //Serial.print("timeout "); Serial.println(timeout_ms);
...
Lidar_Data_Processing::Lidar_Data_Processing(){
  ...
  //uint64_t m_node_time_ns = millis(TIME_NANOSECOND);			  //< time stamp
...
/************************************************************************/
int32_t Lidar_Data_Processing::sendCommand(uint8_t cmd)
{
  uint8_t pkt_header[10];
	...
	switch (cmd)
	{
      ...
      pkt_header[2]=0xF0;
      pkt_header[3]=0x0F;
      Serial2.write(pkt_header,4);
      //Serial.print("wrote "); Serial.print(pkt_header[0], HEX); Serial.print(pkt_header[1], HEX); Serial.print(pkt_header[2], HEX); Serial.println(pkt_header[3], HEX);
...
int32_t Lidar_Data_Processing::sendData(const uint8_t *data, size_t size)
{
...
{
  Serial.println("lidar_start_reply start--->");
 
  int64_t startTs = millis();
  int64_t waitTime = 0;
  ...
  while ((waitTime = millis() - startTs) <= timeout){
      ...
      size_t recvSize = 0;
      int32_t ans = waitForData(remainSize, timeout - waitTime, &recvSize);
      if (!IS_OK(ans)) {
        Serial.println("lidar_start_reply waitForData fail---");
        return false;
      }
      if (recvSize >= remainSize) {
        recvSize = remainSize - recvPos;
      }
      ans = Serial2.readBytes(recvBuffer, recvSize);
      if (IS_FAIL(ans)) {
        Serial.println("lidar_start_reply read_data fail---");
        return false;
      }
      ...
      if(checkvalue == checkdata && recvPos==12){
        Serial.print("lidar_start_reply--------OK");
        Serial.print(checkvalue); Serial.print(","); Serial.println(checkdata);
        return true;
        break;
      }
    }
    Serial.println("lidar_start_reply-----------error");
    return false;
}

int32_t Lidar_Data_Processing::waitSpeedRight(uint8_t cmd,uint64_t timeout)
{
    int  recvPos     = 0;
    ...
    while ((waitTime = millis() - startTs) <= timeout){
      ...
      ans = Serial2.readBytes(recvBuffer, recvSize);
      if (IS_FAIL(ans)) {
        Serial.println("read waitResponseHeader fail");
        return RESULT_FAIL;
      }
      for (size_t pos = 0; pos < recvSize; ++pos) {
        uint8_t currentByte = recvBuffer[pos];
        switch (recvPos) {
          case 0:
            if(currentByte == 0xFA){
              Serial.print("head_speed_000="); Serial.println(currentByte);
            }else{
              continue;
            }
            break;
          ...
          case 1:
            if(currentByte == 0xFA){
              Serial.print("head_speed_111="); Serial.println(currentByte);
            }else{
              continue;
            }
            break;
          ...
          case 2:
            if(currentByte == 0xA5){
              Serial.print("head_speed_222="); Serial.println(currentByte);
          ...
          case 3:
            if(currentByte == 0x5A){
              Serial.print("head_speed_333="); Serial.println(currentByte);
              data_check_sum+=currentByte; 
            }else{
              continue;
            }
          ...
          case 4:
            ...
            data_check_sum+=currentByte; 
            Serial.print("head_speed_444="); Serial.println(currentByte);
            break; 
          
          case 5:
            data_lenth += (currentByte * 0x100);
            data_check_sum+=currentByte;
            Serial.print("head_speed_555="); Serial.println(currentByte);
            break;

          case 6:
            check_sum_cal = currentByte;
            Serial.print("head_speed_666="); Serial.println(currentByte);
            break; 
          
          case 7:
            check_sum_cal += (currentByte * 0x100);
            Serial.print("head_speed_777="); Serial.println(currentByte);
            break;
          
          case 8:
            if(currentByte==0x01)
            {
              Serial.print("head_speed_888="); Serial.println(currentByte); Serial.print(" count="); Serial.println(recvPos);
              data_check_sum+=currentByte; 
              head_right = true;
            }
          default:
      ...
      Serial.println("recv_pos---="); Serial.println(recvPos);
      if(recvPos == 9){
        Serial.println("222");
        break;
      }
    ...
    if(recvPos == 9)
    {
      ...
      while ((waitTime = millis() - startTs) <= timeout)
      {
        ...
        Serial2.readBytes(recvBuffer, recvSize);
...
int32_t Lidar_Data_Processing::waitResponseHeader(uint8_t cmd, uint64_t timeout)
{
  int  recvPos     = 0;
  uint32_t startTs = millis();
  ...
  while ((waitTime = millis() - startTs) <= timeout) {
    ...
    for (size_t pos = 0; pos < recvSize; ++pos) {
      uint8_t currentByte = recvBuffer[pos];
      Serial.print("head_byte="); Serial.println(currentByte);
      switch (recvPos) {
        case 0:
  ...
  if(recvPos == 4)
  {
    ...
    while ((waitTime = millis() - startTs) <= timeout)
    {
      ...
      {
            Serial.print("data*="); Serial.println(recvBuffer[pos]);
      ...
      Serial.print("data**"); Serial.print(check_sum_cal); Serial.print(","); Serial.print(recvBuffer[remainSize-1]); Serial.print(","); Serial.println(remainSize);
...
int32_t Lidar_Data_Processing::waitPackage(node_info *node, uint32_t timeout)
{ 
  int recvPos = 0;
  uint32_t startTs = millis();
  uint32_t waitTime = 0;
  ...
  if (package_Sample_Index == 0)
  {
    ...
    while ((waitTime = millis() - startTs) <= timeout)
    {
      ...
      if (!IS_OK(ans))
      {
        //Serial.print("wp - waitForData -> "); Serial.println(ans);
        return ans;
      }
      ...
      Serial2.readBytes(node_lidar.globalRecvBuffer, recvSize);
        ...
        case 2:
          ...
          if ((package_type == CT_Normal) || (package_type == CT_RingStart))
          {
            if (package_type == CT_RingStart)
            {
              ...
              {
                node_lidar.lidar_time.tim_scan_start = millis();
              }else{
                node_lidar.lidar_time.tim_scan_end = millis();
              }
    ...
    if (PackagePaidBytes == recvPos)
    {
      ...
      while ((waitTime = millis() - startTs) <= timeout)
      {
        ...
        if (!IS_OK(ans))
        {
          //Serial.print("wp - waitForData2 -> "); Serial.print(PackagePaidBytes); Serial.print(","); Serial.print(recvPos); Serial.print(","); Serial.print(remainSize); Serial.print(";"); Serial.println(ans);
          return ans;
        }
      ...
      if (package_Sample_Num * PackageSampleBytes != recvPos)
      {
        //Serial.print("wp - recvPos err -> "); Serial.println(recvPos);
        return RESULT_FAIL;
      }
    }else{
      //Serial.print("wp - paidBytes err -> "); Serial.println(recvPos);
      return RESULT_FAIL;
    }
    ...
    {
//      Serial.print("data check,"); Serial.print(CheckSumCal); Serial.print(","); Serial.print(CheckSum); Serial.print(","); Serial.print(package_Sample_Num); Serial.print(","); Serial.println(recvPos);
      CheckSumResult = false;
      has_package_error = true;
    }
    else
...
int32_t Lidar_Data_Processing::waitPackage_coin(node_info *node,uint32_t timeout)
{
...
int32_t Lidar_Data_Processing::waitScanData(node_info *nodebuffer, size_t &count, uint32_t timeout)
{
  ...
  while ((waitTime = millis() - startTs) <= timeout && recvNodeCount < count)
  {
    ...
    if (!IS_OK(ans))
    {
      //Serial.print("waitPackage ->"); Serial.println(ans);
